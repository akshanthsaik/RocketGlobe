...
export function RocketsTab() {
  ...
  const sortedRockets = useMemo(() => {
    let filtered = rockets.filter(rocket => 
          ...
          >
            <div className="rocket-info">
              <div className="rocket-name-row">
                <div className="rocket-name">{rocket.full_name || rocket.name}</div>
                <span className={`status-badge ${rocket.is_active ? 'active' : 'retired'}`}>

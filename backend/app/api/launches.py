...
async def get_upcoming_launches(limit: int = Query(10, le=100), db: Session = Depends(get_db)):
    """Get upcoming launches."""
    now = datetime.utcnow()
    launches = db.query(Launch).filter(Launch.net >= now).order_by(Launch.net.asc()).limit(limit).all()
    return launches
